...
🖼️ **이미지 자리** — CounterV03(증가/감소/초기화 버튼)이 동작하는 화면
...
- **미완성 숙제**: Header에 로그인 상태 토글 버튼을 붙이려다 실패 — 이유를 알아내고 집에서 마저 하기로.
...
즉, **"값이 바뀌면 화면도 같이 바뀌게" 하려면 `let` 변수가 아니라 `useState`가 필요하다** — 오늘 배운 카운터·NameForm이랑 완전히 같은 원리다. 이 부분은 시간이 부족해서 오늘은 여기까지만 하고, 집에서 `useState`로 마저 완성해보기로 했다.
...
> "로그인 버튼 하나 토글하는 게 왜 이렇게 어렵지" 싶었는데, 알고 보니 필요한 개념(useState) 자체를 아직 안 붙였던 거였다. 집에서 마저 끝내고 다음 글에 이어 적어야지.
